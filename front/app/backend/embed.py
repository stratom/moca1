...
# Improved function to read environment variables from .env
...
required_vars = ["VOLUME_PATH", "PORT", "CONTAINER_NAME", "ORACLE_PWD"]
missing_vars = [var for var in required_vars if var not in env_vars]

if missing_vars:
    print(f"❌ Error: Missing variables in .env: {', '.join(missing_vars)}")
    exit()
...
DSN = f"localhost:{PORT}/freepdb1"  # Dynamic port from .env
USER = "sys"
